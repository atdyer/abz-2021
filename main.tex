...
Consistent with these principles, Sterling is designed as a web application,\footnote{A Sterling demo with examples can be found at \href{https://sterling-js.github.io}{https://sterling-js.github.io}} built using React and Redux libraries, and packaged with a custom build of Alloy that is launched automatically in the user's default browser by Alloy when an instance is generated. 
...
The Sterling graph view offers the same functionality of the Alloy graph view, but also provides a few key extensions. 
...
As such, users receive the same kind of immediate visual feedback provided by the graph view, with the added benefit of complete control over the visual approach used to display instances.

\begin{figure}
    \centering
    \frame{\includegraphics[width=0.95\linewidth]{mesh.png}}
    \caption{An Alloy instance of a finite element mesh, rendered as a planar embedding in the Sterling script view. Mesh nodes are rendered as white circles and elements as green triangles.}
...
Additionally, Sterling is the visualizer for an Alloy-like model finder called Forge, developed at Brown University to teach a Logic for Systems class of over 60 students.

...
{\color{red}The transition here needs to be that our prior studies
  motivated what we're doing now, and I wonder about adding (back?) a
  comment about discussions in the Alloy meeting.  Otherwise the
  reader will think we're about to describe other ``prior''
  visualization work instead of work that motivates better
  visualization.  E.g., something like ... Beyond these limitations,
  our own work has demonstrated the need for improved visualization
  and the ability to customize ... no ``print'' statements, and need
  to interpret rich state.}
